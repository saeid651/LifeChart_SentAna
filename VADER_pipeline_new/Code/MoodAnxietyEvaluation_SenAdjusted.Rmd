...
load("sentiment_scores_SentenceNormalized.RData")
...
MeasrdScore_MA <- sentiment_scores_SentenceNormalized[which(sentiment_scores_SentenceNormalized$id%in%SelfReport_MA$id), ]
```
...
pairs.panels(data.frame(pos.score, neg.score, obj.score, pol.score, tot.words, tot.sen), main = "Adjusted score with total sentences")
```
...
- Interviewers effect on dependet scores:
